...
# 目標價（買入區間 + 停損）讀取
# =============================================================================
# 跟 stock_groups.json 是各自獨立的一份檔案：這裡只存「原始輸入」欄位
# （中心價、買入價格(L)%、買入價格(U)%、停損價格），不存算好的買入區間絕對值上下限
# ——絕對值上下限一律由 signal_module/target_price.py 的 compute_buy_zone() 現算，
# 「🎯 目標價編輯」頁面顯示用的絕對值也是即時算出來的，不會寫進這份 JSON。
#
# 格式：{ "2330.TW": {"target_price": 2200, "low_pct": 5, "high_pct": 5, "stop_loss": 2000}, ... }
# stop_loss 沒設定時為 None。沒有出現在這份 JSON 裡的股票，視同完全沒設定目標價
# （signal_module/target_price.py 會直接判定「未設定目標價」，不會誤判為 0）。
#
# 這個檔案（app.py）只負責「讀取」target_price_list.json 供主監控迴圈查表使用；
# 「編輯 / 存檔 / 備份 / 推送 GitHub」都在獨立的 pages/2_🎯_目標價編輯.py 頁面裡，
# 跟股票分組編輯的文字框是完全不同的頁面（Streamlit 的 pages/ 頁面彼此無法互相 import
# 對方模組層級會執行 Streamlit 呼叫的程式碼，所以下面這幾個函式的邏輯，
# 在 pages/2_🎯_目標價編輯.py 裡會有各自獨立的一份，兩邊需要修改時記得同步）。
TARGET_PRICE_FILE = "target_price_list.json"


def _to_bool_target_price(value, default=True):
    """把各種可能的原始輸入正規化成布林值；沒有這個 key（value 為 None）時視為 default
    （沿用舊資料 / 相容沒有 enabled 欄位的既有 target_price_list.json，一律預設開啟）。
    """
    if value is None:
        return default
...
def _normalize_target_price_entry(raw: dict):
    """把一筆原始輸入正規化成標準格式；欄位型別不對就盡量救、救不了就回傳 None（呼叫端應跳過這筆）。"""
    ...
    return {
        ...
        "high_pct": high_pct,
        "stop_loss": stop_loss,
        "enabled": _to_bool_target_price(raw.get("enabled"), True),
    }


def validate_and_normalize_target_price_json(data: dict) -> dict:
    """驗證/正規化整份 target_price_list JSON（讀檔、GitHub 抓取、匯入時共用）。
    格式錯誤的股票會直接跳過，不會讓整份資料讀取失敗。
    """
    if not isinstance(data, dict):
...
def _fmt_target_price(value) -> str:
    """目標價數字顯示用：整數就不顯示小數點，否則顯示到小數第二位，避免像 2200.00 這樣冗長。"""
    try:
        value = float(value)
    except (TypeError, ValueError):
        return str(value)
...
# 目標價（買入區間 / 停損）：這兩個訊號走「彙整後定時 Telegram 推播」+「toast 冷卻」自己
# 獨立的一套通知機制（見 render_live_monitor() 內的目標價彙整區塊），不套用「強勢股達標」
# 那條逐檔即時推播的規則，所以要從 pushable_signal_hits 判斷式裡排除，避免同一檔股票被
# 重複推播兩次（一次強勢股達標、一次目標價彙整）。
# 但這兩個訊號本身依然是正常註冊在 SIGNAL_REGISTRY 的訊號，一樣會出現在監控表格的
# 「買賣訊號」欄位、也會出現在「🛠️ 訊號編輯」頁面的訊號清單裡，只有「要不要觸發強勢股
# 那條推播」這件事被排除。
TARGET_PRICE_LABELS = {"進入買入區間", "觸及停損價格"}
...
def run_stock_signals(symbol, name, df, open_val, high_val, low_val, close_val, rise_threshold=5.0, price_ref_date=None, extra_params=None):
    """
    對單一股票跑過全部已註冊訊號。
    ...
    extra_params：呼叫端可選擇性傳入，會併入 ctx.params（例如這檔股票自訂的目標價設定），
                  讓個別訊號模組（例如 signal_module/target_price.py）可以讀到逐檔不同的資料，
                  不影響其他既有訊號模組（它們不認得的 key 會直接忽略）。
    """
    try:
...
if "target_price_toast_cooldown" not in st.session_state:
    # key: f"{symbol}_{label}" (例如 "2330.TW_進入買入區間") -> 上次跳 toast 的 timestamp（秒）
    # 用來實作「同一檔股票、同一種提醒類型，30 分鐘內只跳一次」的冷卻機制，
    # 跟 Telegram 的定時彙整推播互相獨立（toast 只要冷卻時間到、條件仍成立，每次刷新都可能再跳）。
    st.session_state.target_price_toast_cooldown = {}
...
def render_live_monitor(rise_threshold):
    ...
    # ===== 目標價（買入區間 / 停損）本次掃描要用的查表 + 彙整收集器 =====
    # target_price_map：symbol -> {"target_price","low_pct","high_pct","stop_loss"}，
    # 「🎯 目標價編輯」頁面存檔時會直接更新 st.session_state.target_price_list，
    # 這裡直接引用同一份 dict，不用重新讀檔。
    target_price_map = st.session_state.target_price_list
    # 同一檔股票如果同時存在多個分組，run_stock_signals() 對它還是會分組各跑一次，
    # 這裡用 set 記錄「這次掃描已經收集過的股票」，確保彙整訊息 / toast 每檔股票每次掃描只算一次，
    # 不會因為同一檔股票在兩個分組裡各出現一次就重複收集。
    target_price_seen_this_scan = set()
    # 彙整訊息用：依類型分組收集，最後在整個掃描迴圈跑完後，一次組成一則 Telegram 訊息送出。
    target_price_hits_this_scan = {"buy_zone": [], "stop_loss": []}
    ...
    for group_name, stocks in st.session_state.stock_groups.items():
        ...
        for symbol in stocks:
            try:
                ...
                # 目標價：從 target_price_map 查這檔股票有沒有設定，有的話塞進 extra_params，
                # 讓 signal_module/target_price.py 可以讀到（沒設定就是 None，訊號模組會判定「未設定」）。
                target_price_cfg = target_price_map.get(symbol)
                signal_hits, signal_display = run_stock_signals(
                    ...
                    extra_params={"target_price": target_price_cfg} if target_price_cfg else None,
                )

                # ===== 目標價命中收集：toast（30分鐘冷卻） + Telegram 定時彙整訊息素材 =====
                # 用 signal_hits（尚未套用 GENERALIZED_THREE_METHOD_LABELS 排除規則的完整清單）
                # 判斷，這樣才不會漏掉「同時命中其他訊號」的情況。
                if symbol not in target_price_seen_this_scan:
                    tp_hits = [h for h in signal_hits if h["label"] in TARGET_PRICE_LABELS]
                    if tp_hits:
                        target_price_seen_this_scan.add(symbol)
                        cfg = target_price_cfg or {}
                        code_only = symbol_to_code(symbol)
                        now_ts = tw_now.timestamp()
                        for h in tp_hits:
                            if h["label"] == "進入買入區間":
                                ...
                                target_price_hits_this_scan["buy_zone"].append({
                                    "code": code_only, "name": stock_name,
                                    "price": data["price"], "lower": lower, "upper": upper,
                                })
                                toast_msg = f"🎯 {stock_name}({code_only}) 進入買入區間 {_fmt_target_price(lower)}-{_fmt_target_price(upper)}\n價格：{data['price']}"
                            else:  # "觸及停損價格"
                                stop_loss_val = float(cfg.get("stop_loss", 0) or 0)
                                target_price_hits_this_scan["stop_loss"].append({
                                    "code": code_only, "name": stock_name,
                                    "price": data["price"], "stop_loss": stop_loss_val,
                                })
                                toast_msg = f"⚠️ {stock_name}({code_only}) 觸及停損價格 {_fmt_target_price(stop_loss_val)}\n價格：{data['price']}"

                            cooldown_key = f"{symbol}_{h['label']}"
                            last_toast_ts = st.session_state.target_price_toast_cooldown.get(cooldown_key)
                            if last_toast_ts is None or (now_ts - last_toast_ts) >= 1800:
                                st.toast(toast_msg, duration=300)
                                st.session_state.target_price_toast_cooldown[cooldown_key] = now_ts

                is_high_gain = data["pct"] >= 5
                # 過濾規則：如果命中的訊號「只有」廣義上升三法 / 廣義下降三法，不算數（不觸發推播）；
                # 只要還有其他訊號一起命中，就照樣算數，一併推送。
                # 目標價（進入買入區間 / 觸及停損價格）也一併排除：這兩個訊號走自己獨立的
                # 「Telegram 定時彙整 + toast 冷卻」通知機制（見上方收集區塊），不套用這條
                # 「強勢股達標」逐檔即時推播規則，避免同一檔股票被重複推播兩次。
                pushable_signal_hits = [
                    h for h in signal_hits
                    if h["label"] not in GENERALIZED_THREE_METHOD_LABELS and h["label"] not in TARGET_PRICE_LABELS
                ]
    ...
    # ===== 目標價（買入區間 / 停損）Telegram 定時彙整推播 =====
    # 跟「強勢股達標」逐檔即時推播不同，這裡固定在同一個排程時段（跟強勢股共用同一套
    # can_push_now / TARGET_TIMES 判斷）把「這次掃描」全部符合條件的股票彙整成單一則訊息，
    # 依類型分組（先列全部「進入買入區間」，再列全部「觸及停損」），並帶上時段標籤，
    # 讓使用者滑歷史紀錄時知道這是「當下狀態快照」而不是新事件。
    # 不做額外的「一天只推一次」去重：同一檔股票如果連續幾個時段都還在區間內/低於停損，
    # 就會連續出現在好幾則彙整訊息裡（已跟使用者確認過這是預期行為）。
    if can_push_now:
        buy_zone_hits = target_price_hits_this_scan["buy_zone"]
        stop_loss_hits = target_price_hits_this_scan["stop_loss"]
        if buy_zone_hits or stop_loss_hits:
            if current_schedule_key:
                hhmm = current_schedule_key.split("_")[-1]
                time_label_display = f"{hhmm[:2]}:{hhmm[2:]}"
            else:
                time_label_display = tw_now.strftime("%H:%M")
            tp_msg_lines = [f"🕐 <b>{time_label_display} 目標價彙整</b>", ""]
            for h in buy_zone_hits:
                tp_msg_lines.append(
                    f"{h['code']} 進入買入區間 {_fmt_target_price(h['lower'])}-{_fmt_target_price(h['upper'])}"
                    f"  價格 : {_fmt_target_price(h['price'])}"
                )
            if buy_zone_hits and stop_loss_hits:
                tp_msg_lines.append("")
            for h in stop_loss_hits:
                tp_msg_lines.append(
                    f"{h['code']} 觸及停損價格 {_fmt_target_price(h['stop_loss'])}"
                    f"  價格 : {_fmt_target_price(h['price'])}"
                )
